...
public class AnglePicker extends Region {
        ...
        converter = new StringConverter<Double>() {
            ...
            public Double fromString(final String string) {
                ...
                if (numberString.matches("\\d{0,3}([\\.]\\d{0,1})?")) {
                    return 360 - Double.valueOf(numberString);
                } else {
                    return 0d;
                }
            }
    ...
    public double getAngle() {
        return null == angle ? _angle : angle.get();
    }
    ...
    public void setAngle(final double angle) {
        if (null == this.angle) {
            ...
            text.setText(String.format(Locale.US, "%.0f\u00b0", 360 - _angle));
            text.relocate((size - text.getLayoutBounds().getWidth()) * 0.5, (size - text.getLayoutBounds().getHeight()) * 0.5);
            textField.setText(String.format(Locale.US, "%.0f\u00b0", 360 - _angle));
        } else {
        ...
        if (null == angle) {
            angle = new DoublePropertyBase(_angle) {
                ...
                protected void invalidated() {
                    ...
                    text.setText(String.format(Locale.US, "%.0f\u00b0", 360 - get()));
                    text.relocate((size - text.getLayoutBounds().getWidth()) * 0.5, (size - text.getLayoutBounds().getHeight()) * 0.5);
                    textField.setText(String.format(Locale.US, "%.0f\u00b0", 360 - get()));
                }

                @Override
                public Object getBean() {
                    return AnglePicker.this;
                }
    ...
    public Paint getIndicatorPaint() {
        return null == indicatorPaint ? _indicatorPaint : indicatorPaint.get();
    }
    ...
    public Paint getTextPaint() {
        return null == textPaint ? _textPaint : textPaint.get();
    }
    ...
    private void updateTextField() {
        ...
        if (!text.matches("\\d{0,3}([\\.]\\d{0,1})?")) {
            return;
        }
        if (null != text && !text.isEmpty()) {
            setAngle(Double.parseDouble(textField.getText()));
        }
        textField.setVisible(false);
        textField.setManaged(false);
    }
